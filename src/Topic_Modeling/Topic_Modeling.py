...
def main():
    ...
    dfs = (im.pre_covid_posts_df, im.post_covid_posts_df)
    for df in dfs:
        print("Story Stats: ")
        im.lmw.print_dataset_stats(df['selftext'])

        df_name = df.name

        #remove emojis, apply redditcleaner, removed stop words
        df['Cleaned Submission'] = df['selftext'].apply(im.redditcleaner.clean).apply(remove_emojis).apply(process_s)
        ...
        #topic_words, topic_doc_distributions = im.lmw.quick_train_topic_model(path_to_mallet, path_to_save, 50, training_chunks)
        num_topics = 50

        topics = im.lmw.load_topic_keys(path_to_save+'/mallet.topic_keys.50')
